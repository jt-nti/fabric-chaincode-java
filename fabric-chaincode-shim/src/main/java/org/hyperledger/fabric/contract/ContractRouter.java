    ...
    private Response processRequest(ChaincodeStub stub) {
        ...
        try {
            ...
            if (throwable instanceof TransactionException) {
                String message = throwable.getMessage()==null ? "Error occurred executing transaction" : throwable.getMessage();

                return ResponseUtils.newErrorResponse(message, ((TransactionException) throwable).getPayload());
            } else if (throwable instanceof ContractRuntimeException) {
                return ResponseUtils.newErrorResponse(throwable);
            } else {
                return ResponseUtils.newErrorResponse(new ContractRuntimeException("Error occurred processing transaction", throwable));
            }
        }
    }
